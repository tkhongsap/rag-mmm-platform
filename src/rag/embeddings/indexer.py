...
def _resolve_qdrant_path(raw_path: str) -> Path:
    """Resolve Qdrant path against project root when a relative path is provided."""
    qdrant_path = Path(raw_path)
    if qdrant_path.is_absolute():
        return qdrant_path
    return _get_project_root() / qdrant_path
...
class RAGIndexer:
    ...
    def __init__(
        ...
        self.qdrant_path = _resolve_qdrant_path(raw_qdrant_path)
